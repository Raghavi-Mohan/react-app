...
export default function ContentBlock({text,buttonText,src,children,route}) {
  ...
  return (
    ...
    <div className={`mb-3 ms-5  w-75 defaultmt-5 ${contentBlock ? "textclr" :"plaintext"}`}>
      <h3 className={`fw-bold height-10 mb-3 ms-1 w-auto ${contentBlock ? "olivetextColor" :"plaintext"}`}>{text}</h3>
    </div>
    {children}
    <button className={`mt-2 height-10 mb-4 ${contentBlock ? "mybtn-clr" :"plainbtn"}`} onClick={()=>navigate(route)}>{buttonText}</button>
    </div>
